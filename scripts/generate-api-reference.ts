 ...
 *
 * Reads extension IDs from anima.example.json, dynamically imports each
...
// ── Extension methods — discovered from anima.example.json ─────
function getExtensionIdsFromConfig(): string[] {
  const raw = readFileSync(join(ROOT, "anima.example.json"), "utf-8");
...
async function loadExtensionMethods(): Promise<MethodDef[]> {
  ...
  for (const id of extensionIds) {
    ...
    try {
      ...
      const factory = mod.default as (() => AnimaExtension) | undefined;
...
const content = `# Anima API Reference
